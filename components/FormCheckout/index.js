...
export default function FormCheckout({ tickets }) {
  ...
  // filter payment setiap ada onChange di payment, nanti ambil payment idnya
  useEffect(() => {
    let paymentId = "";
  ...
  const handleSubmit = async () => {
    try {
      const _temp = [];
      // looping tickets jika t._id === ticketId maka push ke _tempt
      tickets.forEach((t) => {
        if (t._id === ticketId) {
          _temp.push({
            ticketCategories: {
            ...
            sumTicket: 1, // sumTicket: 1 karna kita gada button +-
          });
        }
      });
      let payload = {
  ...
  const handleChangePayment = (e, i) => {
    ...
    // ini utk ngerubah checkednya di halaman client
    _temp.forEach((t) => {
