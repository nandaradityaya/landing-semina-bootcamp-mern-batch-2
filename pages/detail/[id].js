...
export default function DetailPage({ detailPage, id }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await getData("api/v1/events"); // ambil data event
    ...
    fetchData();
  }, []); // array kosong supaya dia di jalanin saat pertama kali halaman di render saja

  const router = useRouter();

  // ketika submit kirim ticketId dan organizer
                  ...
                  <>
                    ...
                    {detailPage.stock !== 0 && (
                      <Button
                        variant={"btn-green"}
                        action={
                          () => handleSubmit(ticket._id, detailPage.organizer) // ketika submit kirim ticket._id dan detailPage.organizer, tujuannya untuk ngambil payment
                        }
                      >
                        Join Now
                      </Button>
...
// klo pake getServerSideProps kita bisa gunain context untuk ambil parameter di url
